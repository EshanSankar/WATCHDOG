...
class SafetyChecker(Node):
    def __init__(self):
        ...
        self.xarm_control = 0
        #TODO: watch for a topic that indicates whether teleop or not
        self.xarm_control_sub = self.create_subscription(TYPE, "TOPIC", self.xarm_control_cb, 10)
        ...
        self.OT2_JOINT_THRESHOLD = 0.005 # check if OT2 is moving
        self.OT2_ASSET_POSITION_THRESHOLD = 0.25 # check assets are in correct spots relative to pipette
        self.ASSET_ROTATION_THRESHOLD = 0.1 # check if assets are oriented properlys
    ...
    def asset_cb(self, msg, asset_type):
        self.asset_poses[asset_type] = np.array([msg.pose.position.x, msg.pose.position.y, msg.pose.position.z,
    ...
    def xarm_control_cb(self, msg):
        #TODO
        self.xarm_control = 1 if msg.data == ____ else 0
    # Primary functions
